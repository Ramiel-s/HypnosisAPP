    ...
    `}>
      ...
      <div className="relative z-30 flex flex-col bg-gray-900/80 backdrop-blur-xl border-b border-white/10 shadow-lg rounded-b-2xl transition-all duration-300">
          ...
          {/* Center: Energy Bar & Title */}
           <div className="flex-1 mx-4 flex flex-col justify-center">
              <div className="flex justify-between items-end mb-1">
                ...
                <span className="text-[10px] text-gray-400">{Math.floor(userData.mcEnergy)} / {userData.mcEnergyMax}</span>
              </div>
        ...
        <div 
          ...
          onClick={() => setIsExpanded(!isExpanded)}
        >
          {isExpanded ? <ChevronUp size={14} className="text-gray-500" /> : <ChevronDown size={14} className="text-gray-500 animate-pulse" />}
        </div>

        {/* Expanded Command Center (Stats + Store) */}
        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[32rem] opacity-100' : 'max-h-0 opacity-0'}`}>
