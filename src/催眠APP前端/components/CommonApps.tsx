...
export const HelpApp = ({ onBack }: { onBack: () => void }) => (
  <PageLayout title="帮助中心" onBack={onBack} color="bg-gray-50">
    <div className="space-y-3">
      {['如何获取 MC 点数?', '催眠失败了怎么办?', 'VIP 等级说明', '隐私政策'].map((item, i) => (
        <div key={i} className="bg-white p-4 rounded-xl shadow-sm flex justify-between items-center cursor-pointer active:scale-95 transition-transform">
